$(document).ready(function() {
    ...
    $(document).on('click', '.updatePayroll', function(e) {
        $('.cardCreatePayroll').show(800);
        $('#btnCreatePayroll').html('Actualizar');

        let row = $(this).parent().parent()[0];
        let data = tblPayroll.fnGetData(row);

        sessionStorage.setItem('id_payroll', data.id_payroll);

        $('#employee').val(data.employee);
        $(`#idProcess option[value=${data.process}]`).attr('selected', true);
        ...
        $('#workingHoursDay').val(data.working_hours_day);
        $('#workingDaysMonth').val(data.hours_day);

        $(`#typeFactor option[value=${data.contract}]`).attr('selected', true);
