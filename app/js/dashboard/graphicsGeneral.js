...
graphicTimeProcessByProduct = (data) => {
  ...
  for (i = 0; i < 10; i++) {
    ...
    enlistmentTime.push(data[i].enlistmentTime);
    operationTime.push(data[i].operationTime);

    totalTime[i] = enlistmentTime[i] + operationTime[i];
  }

  /*const productsCostsData = {
    labels: ['Tiempo Enlistamiento', 'Tiempo Operacion'],
    labels: [enlistmentTime, operationTime],
  };*/
