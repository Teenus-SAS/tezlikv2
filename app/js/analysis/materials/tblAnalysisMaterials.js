$(document).ready(function() {
                    ...
                    for (i = 0; participation <= 80; i++) {
                        ...
                        $('.colMaterials').append(
                            `<tr class="col${i + 1} text-center" id="col${i + 1}">
                              <th scope="row">${i + 1}</th>
                              <th scope="row">${r[i].participation.toFixed(1)}%</th>
                              <th id="rawMaterial-${i + 1}">${r[i].material}</th>
                              <th id="quantity-${i + 1}">${r[i].quantity}</th>
                              <th id="currentPrice-${i + 1}">$ ${r[i].cost.toLocaleString('es-ES')}</th>
    ...
    $(document).on('click', '.negotiatePrice', function(e) {
        ...
        if (!unitsmanufacturated) {
            ...
            $(`.negotiatePrice`).removeAttr("readonly");
    });
    ...
    $(document).on('keyup', '#unitsmanufacturated', function(e) {
        ...
        isNaN(unitsmanufacturated) ? unitsmanufacturated = 0 : unitsmanufacturated
        ...
        for (i = 1; i < count + 1; i++) {
            ...
            if (negotiatePrice) {
                ...
                if (isNaN(projectedCost))
                    ...
                    $(`#projectedCost-${i}`).html(`$ ${projectedCost.toLocaleString('es-ES')}`);
            }
            savingsMontly()
            savingsAnnual()
        }
    ...
    savingsMontly = () => {
        ...
        for (i = 1; i < count + 1; i++) {
            ...
            projectedCost = projectedCost.replace('$', '').replace('.', '').replace('.', '').replace(',', '.');
            projectedCost = parseFloat(projectedCost);

            let currentCost = $(`#totalCost-${i}`).html();
            currentCost = currentCost.replace('$', '').replace('.', '').replace('.', '').replace(',', '.');
            ...
            $(`#monthlySavings`).html(`$ ${totalMonthlySavings.toLocaleString('es-ES')}`);
        }
    }
    ...
    savingsAnnual = () => {
        if (isNaN(totalMonthlySavings)) {
            ...
            $('#monthlySavings').val(`$ ${totalMonthlySavings.toLocaleString('es-ES')}`);
            // Calcular ahorro anual
